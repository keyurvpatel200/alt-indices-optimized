...
const FundSizeDensityChart = ({
  ...
  return (
    <div ref={ ref } className={ styles.wrapper } data-theme={ theme }>
      {width - marginLeft - marginRight > 0 ? (
        <svg className={ styles.svg } viewBox={ `0 0 ${width} ${height}` }>
          ...
          <g className={ styles.area }>
            <path
              fill={ `url(#${id}LinearGradient)` }
              d={ areaGenerator(comparedFund) }
            />
            <path
              fill='none'
              className={ styles.areaLine }
              d={ lineGenerator(comparedFund) }
            />
          </g>
          <g className={ styles.slices }>
            {fundDistribution.map((d, i) => (
              ...
              >
                ...
                <rect
                  width={ x(fundThresholds[i + 1]) - x(fundThresholds[i]) }
...
export default FundSizeDensityChart
